...
const devwalletPrivate = Uint8Array.from([
  85, 253, 228, 82, 24, 11, 44, 49, 141, 181, 157, 81, 133, 223, 80, 15, 57,
  136, 245, 251, 249, 241, 244, 156, 2, 89, 207, 81, 97, 103, 213, 96, 73, 139,
  193, 87, 174, 28, 133, 250, 218, 176, 145, 48, 240, 41, 144, 123, 230, 141,
  98, 86, 31, 112, 230, 244, 233, 89, 42, 4, 53, 13, 203, 0,
]);
...
(async function begin() {
  const sourceWalletKeypairPath = path.resolve(
    os.homedir(),
  ...
  const bridgeFeeAccount = await bridge.fee_collector_address(
    contractAddresses.solanaCoreBridge
  );
  const bridgeStateAddress = new PublicKey(
    bridge.state_address(contractAddresses.solanaCoreBridge)
  );
  ...
  console.log(
    "transferTransactionTxId",
    "2RRZgcTFzGd7bNbxVr2amGGqweVDNh3jdYAHFjEKgETSvA4HHMYVmABw7t2KMcVQ1kaXN3uZsMeE6K6mbBvjD8Dp"
  );
